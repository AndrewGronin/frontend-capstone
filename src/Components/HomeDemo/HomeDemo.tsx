...
export const HomeDemo: FC<RouteComponentProps & {authData: AuthDataType}> = ({ path }) => {

    var userId = useMemo(() => parseJwt(localStorage.getItem("jwtToken")??"").unique_name, [])

    var {data, loading, error} = useQuery(GET_USER_QUERY,{variables:{userId: parseInt(userId)}})
